"""
Create a Brainfuck interpreter in WASM. Brainfuck is an esoteric language with
a very simple syntax, which is very hard to program in.
"""
...
def brainfuck2wasm(code):
    ...
    module = Module(
        TypeSection(
            ...
            FunctionSig(['i32']),  # imported func to write chars
            ),
        ImportSection(
            Import('js', 'stdout_write_char', 'function', 1),
            ),
        FunctionSection(0),
        MemorySection((1, 1)),  # 1 page of 64KiB > 30000 minimum for Brainfuck
        StartSection(1),
        CodeSection(
            FunctionDef(['i32'], *instructions)),
        DataSection(),  # no initial data
        )
    return module


if __name__ == '__main__':
    wasm = brainfuck2wasm(EXAMPLE)
